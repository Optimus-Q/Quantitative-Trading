...
class BackTester:
    ...
    def PandLCalculator(self, data_df, pred):
        df = data_df.copy()
        df = round(df, 4)
        df['Signal'] = np.where(df['bidopen'] < df['bidclose'], 1, -1)
        ...
        for ix, sig in zip(df.index, df['Pred']):
            if (sig == 1):
                ret.append((df['bidclose'][ix]-df['bidopen'][ix])*10000*10*self.lots)
            elif (sig == -1):
                ret.append((df['bidopen'][ix]-df['bidclose'][ix])*10000*10*self.lots)
            else:
                ret.append(0)
        df['Return'] = ret
        # P&L Calculator
